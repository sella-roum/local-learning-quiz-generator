...
Base64化によりデータサイズはおおむね増加するため、元PDFを20MBまで許容すると、実際のリクエストpayloadが想定上限を超える可能性がある。
...
- 実ファイル解決時は、`fs.realpath` または同等の方法で物理パスを取得する。
- 取得した物理パスが、正規化済みの `public/documents` ベースパス配下であることを確認する。
...
- `pdfjs-dist` が `package.json` / `package-lock.json` に残っていないことを確認する。
