...
- `opencode.jsonc` または `opencode.json`
- `AGENTS.md`
...
プロジェクトルートに `opencode.jsonc` を追加する。

ただし、実装時点のOpenCodeで `opencode.jsonc` がプロジェクトconfigとして確実に読み込まれることを `opencode debug config` で確認する。もし読み込まれない場合は、公式ドキュメントに合わせて `opencode.json` に変更する。

#### `opencode.jsonc` に入れるもの
...
5. `opencode.jsonc` または `opencode.json` を追加する。
...
- `opencode.jsonc` または `opencode.json` が追加されている。
...
- `opencode.jsonc` / `opencode.json` の権限設定
