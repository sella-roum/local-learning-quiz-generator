...
- `package-lock.json` が存在しない状態で `npm ci` は使えないため、OpenCode環境整備PRでは先に `npm install --package-lock-only` を実行して `package-lock.json` を生成する。
...
`eslint.config.mjs` には、少なくとも以下の `ignores` を含める。
...
`.next/**` は必ず除外する。`eslint .` がNext.jsのビルド成果物をlint対象に含めると大量の不要なエラーが出るためである。
...
### 7. package-lock.jsonを生成する

`package-lock.json` が存在しない場合は、以下を実行して `package-lock.json` を生成する。

```bash
npm install --package-lock-only
```

生成された `package-lock.json` はコミットする。

`package-lock.json` を生成した後に、以下を実行してlockfile前提の検証に切り替える。
...
8. `package-lock.json` が存在しない場合は、`npm install --package-lock-only` を実行して `package-lock.json` を生成する。
9. `eslint.config.mjs` を追加し、`ignores` に `.next/**` / `node_modules/**` / `dist/**` / `out/**` / `coverage/**` を含める。
10. `.github/workflows/ci.yml` を追加する。
11. `.env.example` を追加する。
12. `.gitignore` を修正する。
13. READMEに指定文面を追記する。
14. `npm ci` を実行する。
15. `npm run check` を実行する。
16. `npm run check` が失敗したときは、`npm run typecheck` / `npm run lint` / `npm run build` を個別実行して原因を切り分ける。
17. OpenCode CLI がインストール済みの環境では `opencode debug config` を実行する。
18. OpenCode CLI が未インストールの環境では、PR本文のNotesに `OpenCode CLI not available` と記載する。
19. 実行できなかった検証があるときは、PR本文に理由と次アクションを書く。
...
- `package-lock.json` が追加されている。
- `eslint.config.mjs` が追加されている。
- `eslint.config.mjs` の `ignores` に `.next/**` / `node_modules/**` / `dist/**` / `out/**` / `coverage/**` が含まれている。
...
- `package-lock.json` を生成して `npm ci` 前提を成立させる
...
- [ ] npm install --package-lock-only（`package-lock.json` が存在しなかった場合）
- [ ] npm ci
- [ ] npm run check
...
- `eslint.config.mjs` で `.next/**` を除外していることを明記する
```
...
- `package-lock.json` が追加され、`npm ci` が実行可能になっているか
- `eslint.config.mjs` の `ignores` に `.next/**` が含まれているか
- CIが現実に動くか
