...
- 上限値は `src/lib/limits.ts` などの定数ファイルに集約する。

### 初期上限案

最初の実装では、以下を仮上限として採用する。実運用で失敗率やコストを見ながら調整する。
...
このタスクはAPI周りの品質改善だが、Gemini API共通化、ログ抑制、環境変数化と密接に関係するためP1として扱う。
...
- TASK-013のGemini API共通化と整合する形にする。
...
# Optional. Leave empty to use the built-in fallback list.
GEMINI_FALLBACK_MODEL=
...
- ユニットテストは原則として **Vitest** を採用する。
- E2Eは **Playwright** を採用する。
- Reactコンポーネントテストが必要になった場合のみ Testing Library の追加を検討する。
- API routeのテスト方法を決める。
...
## TASK-043: Dependabot auto-merge の前提条件と安全条件を整備する

### 背景

Dependabot auto-merge workflow では `DEPENDABOT_MERGE_TOKEN` を利用している。secret未設定の場合は動作しない。

確認だけで終わらせず、安全に自動マージできる条件を明確化する。

### 対応内容

- `DEPENDABOT_MERGE_TOKEN` の必要性をREADMEまたはdocsに記載する。
- `DEPENDABOT_MERGE_TOKEN` が未設定でも失敗理由が分かるようにする。
- CI成功をauto-mergeの前提にする。
- auto-merge対象を patch / minor のみに制限する。
...
- CI未成功の依存更新が自動マージされない。
- major updateが自動マージされない。
- secret未設定時の失敗理由が追いやすい。
