...
このアプリは、あなたが持っているテキストファイル、画像ファイル、PDF ファイル、または Web ページの URL をもとに、AI が自動で 4 択クイズを作成してくれる学習ツールです。自分で作った資料や気になった Web ページから手軽に復習用のクイズが作れるので、効率的に知識を定着させることができます。

**【！】注意点**

- **AI の利用について**: クイズを作るために、Google の AI「Gemini」を使っています。AI の学習に利用される可能性があるので、**大切な個人情報や会社の機密情報などはアップロードしないでください**。Web ページの内容を取得する際も同様です。
- **データの保存場所**: 作成したクイズや学習の記録は、**今使っているブラウザの中 (IndexedDB) に保存されます**。他のパソコンやスマホで同じデータを使いたい場合は、「エクスポート」と「インポート」機能を使ってデータを移動させてください。
- **URL からのコンテンツ取得について**: Web ページのコンテンツ取得には外部サービス（Jina Reader）を利用しています。サイトの構造や制限によっては、うまく内容を取得できない場合があります。
...
- **できること**:
  - **学習の状況**: 登録したファイルの数、作ったクイズの数、挑戦した回数などがわかります。
...
- **ここは何をするところ？**: クイズを作るための元になるファイルをアップロードしたり、Web ページのコンテンツを取得したり、それらを管理する場所です。
- **できること**:
  - **コンテンツの追加**:
    - **ファイルアップロード**: 「ファイルアップロード」タブで、テキスト (`.txt`)、画像 (`.jpg`, `.jpeg`, `.png`)、PDF (`.pdf`) ファイルをアップロードできます。点線で囲まれたエリアにファイルをドラッグ＆ドロップするか、「ファイルを選択」ボタンで選びます。
    - **URL から取得**: 「URL から取得」タブに切り替えて、Web ページの URL を入力し「取得」ボタンを押すと、そのページのテキストコンテンツを取得してテキストファイルとして登録できます。（※ サイトによっては取得できない場合があります）
    - アップロードまたは URL 取得後、AI がファイルの中身を読み取って、キーワードや簡単なまとめを作ってくれます（少し時間がかかることがあります）。進捗バーが表示されます。
  - **ファイル一覧**: 登録したファイル（アップロードしたもの、URL から取得したもの）がカード形式で表示されます。ファイル名やキーワード（一部）が見えます。
  - **ファイル操作**:
    ...
    - **すべて削除**: 右上の「すべてのファイルを削除」ボタンで、登録したファイルを全部消せます（確認メッセージが出ます）。

**3. ファイル詳細 (`/files/[ファイルID]`)**

![image](./images/files-fileId.png)

- **ここは何をするところ？**: 登録したファイルの中身や、AI が解析した結果を詳しく見る場所です。
- **できること**:
  - **ファイル情報**: ファイル名、サイズ、登録日時などがわかります。
  - **AI 解析結果**: AI が考えたファイルの「概要」「キーワード」「構成」が表示されます。
  - **プレビュー/テキスト**: 画像ならプレビュー、テキストや PDF、URL から取得したコンテンツなら抽出された文字が表示されます。
  - **ダウンロード**: 元のファイルをダウンロードできます（URL から取得した場合はテキストファイルとしてダウンロードされます）。
...
- **ここは何をするところ？**: 1 つのファイル（アップロードまたは URL から取得したもの）から AI を使ってクイズを作る場所です。
- **できること**:
...
- **テーマ切り替え**: 画面右上の 太陽 / 月 のアイコンで、画面の見た目（ライトモード/ダークモード）を切り替えられます。
- **ホーム画面に追加 (PWA)**: 対応しているブラウザ（Chrome, Edge など）では、このアプリをスマホやパソコンのホーム画面に追加して、普通のアプリのように使うことができます。

これで基本的な操作はバッチリなはずです！AI と一緒に楽しく学習を進めてください！
