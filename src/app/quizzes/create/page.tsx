...
import { generateQuizzes, GenerateQuiz } from "@/lib/api-utils";
...
export default function CreateQuizPage() {
    ...
    try {
      ...
      const quizzes = await generateQuizzes(
        ...
        file.keywords,
        {
          ...generationOptions,
          category:
            generationOptions.category === "新規作成"
              ? newCategory
      ...
      if (quizzes.length > 0) {
        ...
        setEditedQuiz({
          ...
          options: quizzes[0].options.map((option) => option.toString()),
          correctOptionIndex: quizzes[0].correctOptionIndex,
  ...
  const handleSaveQuiz = useCallback(async () => {
    ...
    try {
      ...
      // 次のクイズがあれば表示、なければクイズ一覧に戻る
      if (currentQuizIndex < generatedQuizzes.length - 1) {
        setCurrentQuizIndex(currentQuizIndex + 1);
        const nextQuiz = generatedQuizzes[currentQuizIndex + 1];
        setEditedQuiz({
          category:
            nextQuiz.category ||
            (generationOptions.category === "新規作成"
          ...
          options: nextQuiz.options.map((option) => option.toString()),
          correctOptionIndex: nextQuiz.correctOptionIndex,
  ...
  }, [currentQuizIndex, generatedQuizzes, generationOptions, newCategory]);
          ...
          <div className="space-y-6">
                                ...
                                {categories?.map((category) => (
                                  ...
                                  >
                                    <Check
                                      className={cn(
                                        "mr-2 h-4 w-4",
                                        generationOptions.category === category
                                          ? "opacity-100"
                                          : "opacity-0"
                                      )}
                                    />
            ...
            {generatedQuizzes.length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle>
                    クイズ編集 ({currentQuizIndex + 1}/{generatedQuizzes.length}
                    )
                  </CardTitle>
                </CardHeader>
                <CardContent>
